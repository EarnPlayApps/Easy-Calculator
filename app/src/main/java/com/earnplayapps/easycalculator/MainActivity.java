...
public class MainActivity extends Activity {
 ...
 @Override protected void onCreate(Bundle b){super.onCreate(b);ui();handler.postDelayed(()->{if(!isFinishing()&&!isDestroyed())safeConsent();},2500);}
 private void ui(){
 ...
 private void ads(){if(ads||consent==null||!consent.canRequestAds()||isFinishing()||isDestroyed())return;ads=true;try{MobileAds.initialize(this,s->{if(isFinishing()||isDestroyed())return;try{loadBanner();loadInter();loadReward();}catch(Exception ignored){}});}catch(Exception ignored){ads=false;}}
 private void loadBanner(){if(!ads||banner==null||bannerLoading||isFinishing()||isDestroyed())return;try{bannerLoading=true;banner.setAdSize(getAdaptiveBannerSize());banner.loadAd(new AdRequest.Builder().build());}catch(Exception e){bannerLoading=false;banner.setVisibility(View.GONE);}}
