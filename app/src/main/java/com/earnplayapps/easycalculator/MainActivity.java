...
public class MainActivity extends Activity {
 ...
 private void ui(){
  ...
  banner=new AdView(this);banner.setAdUnitId(B);banner.setAdListener(new AdListener(){@Override public void onAdLoaded(){bannerLoading=false;banner.setVisibility(View.VISIBLE);}@Override public void onAdFailedToLoad(LoadAdError e){bannerLoading=false;banner.setVisibility(View.VISIBLE);handler.postDelayed(()->loadBanner(),10000);}});banner.setAdSize(getAdaptiveBannerSize());banner.setVisibility(View.VISIBLE);root.addView(banner,new LinearLayout.LayoutParams(-1,-2));
 ...
 private void loadBanner(){if(!ads||banner==null||bannerLoading||isFinishing()||isDestroyed())return;try{bannerLoading=true;banner.setVisibility(View.VISIBLE);banner.setAdSize(getAdaptiveBannerSize());banner.loadAd(new AdRequest.Builder().build());}catch(Exception e){bannerLoading=false;handler.postDelayed(()->loadBanner(),10000);}}
